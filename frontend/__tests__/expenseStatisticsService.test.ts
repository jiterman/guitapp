...
describe('expenseStatisticsService', () => {
  ...
  describe('getFixedAndVariableStatistics', () => {
    it('should fetch fixed vs variable statistics with period only', async () => {
      const mockToken = 'mock-token';
      const mockResponse = {
        totalAmount: 1000,
        fixedAmount: 600,
      ...
      const params: ExpenseStatisticsParams = { period: 'monthly' };
      const result = await expenseStatisticsService.getFixedAndVariableStatistics(params);

      expect(authService.getToken).toHaveBeenCalled();
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/expenses/statistics/fixed-variable?period=monthly'),
        expect.objectContaining({
          method: 'GET',
          headers: {
            Authorization: `Bearer ${mockToken}`,
    ...
    it('should fetch fixed vs variable statistics with year and month', async () => {
      const mockToken = 'mock-token';
      const mockResponse = {
        totalAmount: 500,
        fixedAmount: 250,
    ...
    it('should throw error when fixed vs variable response is not ok', async () => {
      const mockToken = 'mock-token';
      const mockError = {
        code: 'ERROR_CODE',
        message: 'Error message',
