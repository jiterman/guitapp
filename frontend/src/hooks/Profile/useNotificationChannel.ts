...
export const useNotificationChannel = ({ user, setUser, onSuccess }: Params) => {
  ...
  const handleSave = async (channel: NotificationChannel) => {
    ...
    try {
      const updated = await userService.updateNotificationChannel(channel);

      setUser({
        ...user,
        notificationChannel: updated.notificationChannel ?? channel,
      });
